...
const CharacterCard = React.memo(({ char, isUnlocked, onToggleTrade, onDelete, isTagged }) => (
  <div className="group relative bg-[#1a202c] rounded-xl border border-slate-800 hover:border-pink-500/50 transition-all overflow-hidden shadow-lg">
    ...
    <div className="p-2.5">
      <h4 className="text-[10px] font-black text-white truncate uppercase">{char.name}</h4>
...
export default function MudaeHub() {
  ...
  const smartFixer = async () => {
    ...
    if (!targets.length) return alert("Everything is fixed!");
    setIsFixing(true);
  ...
  return (
    <div className="min-h-screen bg-[#0b0f1a] text-slate-300 flex flex-col md:flex-row">
      <aside className="w-full md:w-56 bg-[#0f172a] border-r border-slate-800 p-5 space-y-6 shrink-0">
        ...
        <nav className="space-y-1">
          ...
          {Object.keys(profiles).map(name => (
            <button key={name} onClick={() => setActiveProfile(name)} className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-xs font-bold transition-all ${activeProfile === name ? 'bg-pink-600 text-white shadow-lg' : 'text-slate-400 hover:bg-white/5'}`}>
              ...
              {isUnlocked && activeProfile === name && <Trash2 size={12} className="opacity-40 hover:opacity-100" onClick={(e) => { e.stopPropagation(); deleteDoc(doc(db, "profiles", name)); }}/>}
            </button>
          ))}
      ...
      <main className="flex-1 p-4 md:p-10 overflow-y-auto">
        <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button onClick={smartFixer} disabled={!isUnlocked || isFixing} className="bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all shadow-lg shadow-blue-500/20">
              {isFixing ? <RefreshCw size={12} className="animate-spin"/> : <CheckCircle2 size={12}/>}
              {isFixing ? `${progress.current}/${progress.total}` : 'Fix Series'}
            </button>
          </div>
        </header>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-[#111622] border border-slate-800 p-5 rounded-2xl space-y-4 shadow-xl">
              ...
              <textarea className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-[10px] h-32 outline-none font-mono focus:border-pink-600" placeholder="Paste $mms l- k..." value={inputText} onChange={(e) => setInputText(e.target.value)} />
