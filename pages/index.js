...
export default function MudaeHub() {
  ...
  // TESTED PARSER FOR NEW $mmsl-ky+a FORMAT
  const handleImport = async () => {
    ...
    const news = lines.map(l => {
      ...
      // Detect Series Line: e.g. "Mato Seihei no Slave - 13/29"
      if (l.includes(' - ') && l.match(/\d+\/\d+/)) {
        currentSeries = l.split(' - ')[0].trim();
        return null;
      }

      // Detect Character Line: starts with #
      if (l.startsWith('#')) {
        const rankMatch = l.match(/#([\d,]+)/);
        const kakeraMatch = l.match(/([\d,]+)\s*ka/);
        const keysMatch = l.match(/\((\d+)\)/); // Finds (4) or (1)

        if (!kakeraMatch) return null;

        // Extract Name: everything between "#Rank - " and the Bullet/Kakera
    ...
    setInputText('');
    alert(`Imported ${news.length} characters!`);
  };
