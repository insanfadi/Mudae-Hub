...
export default function MudaeHub() {
  ...
  const smartFixer = async (forceAll = false) => {
    ...
    // FORCE ALL scan if requested, otherwise only unknowns
        ...
        <header className="flex flex-col xl:flex-row justify-between gap-10 mb-20 items-center">
          <div className="text-center xl:text-left">
            ...
            <div className="flex gap-8 mt-6 justify-center xl:justify-start items-center">
              ...
              <div className="flex items-center gap-4"><button onClick={() => setSortMode(sortMode === 'kakera' ? 'name' : 'kakera')} className="text-[15px] font-black text-slate-400 uppercase flex items-center gap-2 hover:text-pink-500 transition-all"><ArrowUpDown size={18}/> {sortMode === 'kakera' ? 'Kakera' : 'Name'}</button></div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-6 justify-center">
            ...
            <div className="flex flex-col gap-2">
              ...
              {isUnlocked && !isFixing && <button onClick={() => {if(confirm('Fix wrong series for ALL characters?')) smartFixer(true)}} className="text-[10px] font-black text-slate-600 hover:text-pink-500 uppercase tracking-tighter flex items-center justify-center gap-2 transition-all"><Zap size={10}/> Full Rescan (Fix Aqua)</button>}
            </div>
          </div>
        </header>
