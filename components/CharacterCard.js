...
const CharacterCard = ({ char, isUnlocked, onToggleTrade, onDelete, isTagged }) => {
  ...
  // We add a timestamp query to force refresh when the user rescans
  const imgUrl = `/api/mudae?name=${encodeURIComponent(char.name)}&series=${encodeURIComponent(char.series || '')}&v=1`;
