...
// Optimized Character Card with Image Error Handling
const CharacterCard = React.memo(({ char, isUnlocked, onToggleTrade, onDelete, isTagged }) => {
  const [imgError, setImgError] = useState(false);
  const displayImg = imgError 
    ? `https://via.placeholder.com/225x350?text=${encodeURIComponent(char.name)}` 
    : `/api/mudae?name=${encodeURIComponent(char.name)}`;
...
export default function MudaeHub() {
  ...
  const smartFixer = async () => {
    ...
    if (targets.length === 0) return alert("Harem is already fully synced!");
    ...
    for (let i = 0; i < targets.length; i += 5) {
      ...
      await Promise.all(batch.map(async (char) => {
        try {
          ...
          const index = allChars.findIndex(c => c.id === char.id);
          if (index !== -1 && data.series && !data.series.toLowerCase().includes('unknown')) {
            allChars[index].series = data.series;
          }
        } catch (e) {}
      }));
      
      setProgress(i + batch.length);
      // Batch update to Firestore every 20 chars to prevent lag
      if (i % 20 === 0) {
    ...
    <div className="min-h-screen bg-[#0b0f1a] text-slate-300 flex flex-col md:flex-row">
          ...
          {Object.keys(profiles).map(name => (
            <button key={name} onClick={() => setActiveProfile(name)} className={`w-full flex items-center justify-between px-4 py-3 rounded-xl text-xs font-black transition-all ${activeProfile === name ? 'bg-pink-600 text-white shadow-xl' : 'text-slate-400 hover:bg-white/5'}`}>
              ...
              {isUnlocked && activeProfile === name && (
                <Trash2 size={14} className="opacity-40 hover:opacity-100" onClick={(e) => { e.stopPropagation(); if(confirm('Delete Profile?')) deleteDoc(doc(db, "profiles", name)); }}/>
              )}
            </button>
          ))}
          <button onClick={() => { const n = prompt("Friend's Name?"); if(n) setDoc(doc(db, "profiles", n), { characters: [], tradeTags: [] }); }} className="w-full border border-dashed border-slate-700 hover:border-pink-600 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-500 mt-6">+ Add Friend</button>
        </nav>
      </aside>
      ...
      <main className="flex-1 p-4 md:p-12 overflow-y-auto">
        <header className="flex flex-col xl:flex-row justify-between gap-8 mb-16 items-center">
          <div>
            <h2 className="text-7xl font-black text-white italic uppercase tracking-tighter leading-none">{activeProfile || 'Select Profile'}</h2>
            <div className="flex gap-4 mt-4 justify-center xl:justify-start">
              ...
              <button onClick={() => setSortMode(sortMode === 'kakera' ? 'name' : 'kakera')} className="text-[10px] font-black text-pink-500 uppercase tracking-[0.3em] flex items-center gap-2">
                <ArrowUpDown size={10}/> SORT: {sortMode}
              </button>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-4 justify-center">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl flex p-2 shadow-2xl">
              ...
              <button onClick={() => setIsUnlocked(pin === "1234")} className={`p-2.5 rounded-xl transition-all ${isUnlocked ? 'bg-green-500 text-white shadow-lg' : 'text-slate-600 hover:bg-white/5'}`}>
            ...
            <button 
              ...
              className="bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 text-white px-8 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest flex items-center gap-3 shadow-xl shadow-blue-500/20"
            >
              {isFixing ? <RefreshCw size={16} className="animate-spin"/> : <CheckCircle2 size={16}/>}
              {isFixing ? `FIXING: ${progress} / ${sortedChars.length}` : 'Smart Sync Harem'}
            </button>
          </div>
        </header>
          ...
          <div className="lg:col-span-1 space-y-8">
            <div className="bg-[#111622] border border-slate-800 p-6 rounded-3xl space-y-6 shadow-2xl sticky top-10">
              <div className="relative">
                ...
                <input className="w-full bg-slate-950 border border-slate-800 rounded-2xl py-4 pl-12 pr-4 text-xs outline-none focus:border-pink-500 font-bold" placeholder="Search Harem..." onChange={(e) => setSearch(e.target.value)} />
              </div>
              <div className="space-y-4">
                <textarea 
                  className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-[10px] h-40 outline-none font-mono focus:border-pink-600" 
                  placeholder="Paste $mms l- k here..." 
                  value={inputText} 
                  onChange={(e) => setInputText(e.target.value)} 
                />
                <button onClick={handleImport} disabled={!isUnlocked} className="w-full bg-pink-600 hover:bg-pink-500 py-4 rounded-2xl text-xs font-black text-white uppercase tracking-widest transition-all shadow-xl shadow-pink-600/30">
                  Import Data
                </button>
              </div>
            </div>
            ...
            {sortedChars.map((c) => (
              <CharacterCard 
                ...
                onToggleTrade={(s) => {
                  ...
                  const isTagged = currentTags.includes(s);
                  updateDoc(doc(db, "profiles", activeProfile), { 
                    tradeTags: isTagged ? arrayRemove(s) : arrayUnion(s) 
                  });
                }} 
